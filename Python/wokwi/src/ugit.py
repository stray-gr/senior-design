...
# NOTE This is GPL v3 code
# TODO Look into how using this affects your project (may need to GPL v3 it as well)
# TODO Submit a pull request if the old version really is causing issues
...
# TODO Improve ilistdir handling (pattern match the 4-tuple)
...
# TODO Improve ilistdir handling (update args to use tuple elements)
def add_to_tree(dir_item):
  global internal_tree
  if is_directory(dir_item) and len(os.listdir(dir_item[0])) >= 1:
    os.chdir(dir_item[0])
    for i in os.ilistdir():
      add_to_tree(i)
    os.chdir('..')
    ...
    print(dir_item)
    if os.getcwd() != '/':
      subfile_path = os.getcwd() + '/' + dir_item[0]
    else:
      subfile_path = os.getcwd() + dir_item[0]
    try:
...
# TODO Inline this??? If not, update the args to use the correct tupple element
def is_directory(file):
    return (file[1] == 0x4000) # 0x4000 is dir, 0x8000 is file
